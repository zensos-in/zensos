...
/**
 * POST /api/admin/recover-env
 * Captures all current process.env values and saves them into the
 * "recovered" MongoDB collection. Protected by adminAuth.
 * Use this only when you have lost your local .env file and need to
 * recover keys that are set in Vercel/production environment variables.
 */
const RecoveredEnv = require("../models/RecoveredEnv");

router.post("/recover-env", adminAuth, async (req, res) => {
  try {
    // Collect all environment variables as a plain object
    const envSnapshot = {};
    for (const [key, value] of Object.entries(process.env)) {
      envSnapshot[key] = value || "";
    }

    const doc = await RecoveredEnv.create({
      capturedAt: new Date(),
      capturedBy: req.adminUsername || "admin",
      env: envSnapshot,
    });

    return res.json({
      message: "Environment variables captured and saved to 'recovered' collection.",
      id: doc._id,
      capturedAt: doc.capturedAt,
      totalKeys: Object.keys(envSnapshot).length,
    });
  } catch (error) {
    console.error("[recover-env error]", error);
    return res.status(500).json({ message: "Failed to capture environment variables.", detail: error.message });
  }
});

/**
 * GET /api/admin/recover-env/latest
 * Returns the most recently saved env snapshot from the "recovered" collection.
 * Protected by adminAuth.
 */
router.get("/recover-env/latest", adminAuth, async (req, res) => {
  try {
    const RecoveredEnv = require("../models/RecoveredEnv");
    const latest = await RecoveredEnv.findOne().sort({ capturedAt: -1 });

    if (!latest) {
      return res.status(404).json({ message: "No recovered env snapshot found. Run POST /api/admin/recover-env first." });
    }

    // Convert Map to plain object for JSON response
    const envPlain = {};
    for (const [key, value] of latest.env.entries()) {
      envPlain[key] = value;
    }
    ...
    return res.json({
      ...
      capturedBy: latest.capturedBy,
      totalKeys: Object.keys(envPlain).length,
      env: envPlain,
    });
  } catch (error) {
    console.error("[recover-env/latest error]", error);
    return res.status(500).json({ message: "Failed to fetch env snapshot.", detail: error.message });
  }
});
